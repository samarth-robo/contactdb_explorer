...
# Modified by Samarth Brahmbhatt to work with io.BufferedReader objects
...
class PlyElement(object):
    ...
    def _read(self, stream, text, byte_order):
        ...
        hasfileno = is_fileobject(stream)
        if text:
            ...
            if isinstance(stream, io.BufferedReader):
              stream.seek(offset)
              self._data = _np.frombuffer(stream.read(self.count*dtype.itemsize),
                  dtype, count=self.count)
            else:
...
class PlyListProperty(PlyProperty):
    ...
    def _read_bin(self, stream, byte_order):
        ...
        try:
            if isinstance(stream, io.BufferedReader):
              n = _np.frombuffer(stream.read(_np.dtype(len_t).itemsize), len_t, 1)[0]
            else:
        ...
        if isinstance(stream, io.BufferedReader):
          data = _np.frombuffer(stream.read(n*_np.dtype(val_t).itemsize), val_t, n)
        else:
